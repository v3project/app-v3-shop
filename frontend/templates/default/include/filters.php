...
JS
    ...
    <div class="sort">
        ...
        <div class="vals">
            <a href="#" class="sx-filter-action <?= $filters->sort == '-popular' ? "active" : ""; ?>"
               data-filter="productfilters-sort" data-filter-value="-popular">Популярные</a>
            <a href="#" class="sx-filter-action <?= $filters->sort == 'price' ? "active" : ""; ?>"
               data-filter="productfilters-sort" data-filter-value="price">Сначала дешевые</a>
            <a href="#" class="sx-filter-action <?= $filters->sort == '-price' ? "active" : ""; ?>"
               data-filter="productfilters-sort" data-filter-value="-price">Сначала дорогие</a>
            <a href="#" class="sx-filter-action <?= $filters->sort == '-published_at' ? "active" : ""; ?>"
               data-filter="productfilters-sort" data-filter-value="-published_at">Новинки</a>
        </div>
    ...
    <div class="sx-view-mode pull-right">
        ...
        ?>
        <a class="btn btn-default btn-xs <?= \Yii::$app->settings->product_view == 'table' ? "active" : "" ?>"
           href="<?= $urlTable; ?>" data-toggle="tooltip" title="Отображать товары таблицей">
            <i class="glyphicon glyphicon-th" style="padding-right: 0px;"></i>
        </a>
        <a class="btn btn-default btn-xs <?= \Yii::$app->settings->product_view == 'list' ? "active" : "" ?>"
           href="<?= $urlList; ?>" data-toggle="tooltip" title="Отображать товары списком">
